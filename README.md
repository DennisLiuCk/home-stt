...
- Polish 階段沒被浪費。長文等待的本質瓶頸是「按完才開始 ASR」的批次架構,要根本縮短需要 streaming(v0.8.0 候選,見 `plan-v0.8.0-asr-latency.md`)
...
v0.7.x 系列做了大量加速 / 品質研究,以下路徑都 **bench-validated 失效** 或 **證實不適用 Windows + RTX 5080**。未來想繼續優化的人不要再走。詳見 `plan-v0.8.0-asr-latency.md`(若存在)+ commit `8bb6157` (v0.7.1)。
